...
The Yield floor on a Soul's Permit is a hard pre-ranking gate — applied before composite scoring. The Exchange will not rank or deliver any Offer whose bid falls below the Soul's floor for that Signal Type. The floor is adjustable only via Permit settings, not overridden per-Offer.
...
Notification frequency is Soul-configurable per Permit: maximum Offers per day, quiet hours. The Exchange respects these as hard caps before delivery. Default on signup is 1 push notification per day to prevent cold-start spam.

When a Cultivation run produces multiple matched Offers, only the top-ranked Offer (by composite score) is pushed immediately. Remaining matched Offers sit silently in the Soul's feed, discoverable on app open.
...
- Composite ranking requires Conviction and recency signals to be available at match time — these must be stored server-side as outputs of Cultivation (scores only, not raw data).
